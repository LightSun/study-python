...
#       主要用于处理分类.也是常用数据挖掘算法。
'''''
CART(classification and regression tree)的算法整体过程和上面的差异不大(相比c4.5/ID3)，
然是CART的决策是二叉树的每一个决策只能是“是”和“否”，换句话说，即使一个feature有多个可能取值，
也只选择其中一个而把数据分类两部分而不是多个，这里我们主要讲一下分类树，它用到的是基尼指数：
'''
